...
FILES is an alist whose elements are (FILENAME LINENUMBER)."
  (if (boundp 'epoch::version) 
      (progn
	(epoch::select-screen 
	 (epoch::create-screen target-buffer 
			  (list (cons 'title
				      (concat "Server Screen"
					      (sys-name))))))
	(setq *server-screen* (epoch::current-screen))))
  (let (client-record)
    (while files
      (save-excursion
  ...
  (if (or arg
    ...
    (let ((res (server-done)))
	  ...
	  (if (and (boundp 'epoch::version) 
		   (boundp '*server-screen*)) (remove-screen *server-screen*)))
      (server-switch-buffer res))))
