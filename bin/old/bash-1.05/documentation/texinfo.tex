%%%% TeX macros to handle texinfo files: special test version for
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Please use this test version of texinfo.tex to format the Emacs Lisp %
% Reference Manual.  This version of texinfo.tex is derived from       %
% version 1.23, but it has new features, such as the @def.., @result,  %
% and @titlespec commands.  Eventually, this version will be merged    %
% with the distributed texinfo.tex.                                    %
% Thank you     bob@wheaties.ai.mit.edu     23 June 1989               %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\def\texinfoversion{for Elisp Ref Manual v: +.04}  
...
\def\iftex{}
\def\Eiftex{}
% Want to allow \outer control sections, so @ is nothing special here.
\def\ifinfo{\begingroup \catcode`\@ = 12 \ifinfoxxx}
...
% @kbd is like @t, except that if the argument is just one @key command, 
...
% An alternate way to do a titlepage.

% Space at the beginning of the title page.
...
\def\titlespec{\begingroup \parindent=0pt \textfonts
   \let\subtextrm = \tenrm
   \def\subtextfont{\subtextrm \normalbaselineskip = 12pt \normalbaselines}%
   %
   \font\authorrm = cmbx10 scaled \magstep2
   \def\authorfont{\authorrm \normalbaselineskip = 16pt \normalbaselines}%
   %
   % The first paragraph should have some space before it, but all
   % should be ragged left.
   ...
   \leftline{\titlefont{\thistitle}}%
   \vskip5pt
   \hrule height 4pt
   \vskip4pt
   %
   % Now they're supposed to put more text with `@subtext'.
   \def\subtext{\parsearg\subtextzzz}%
   \def\subtextzzz##1{{\subtextfont \rightline{##1}}}%
   %
   ...
   %  
   % Title ``pages'' are actually two pages long (by convention)
   % with space at the top of the second. We don't want the ragged 
   % left with text on the second page.
   \let\oldpage = \page
   \def\page{\vskip5pt \hrule height 2pt \vskip\titlepagebottomglue
      \oldpage \endgroup\hrule height0pt\relax}%
}


\def\Etitlespec{\endgroup\page\HEADINGSon}
...
%\let\ptexequiv=\equiv
%\def\equiv{$\mathord{}\ptexequiv\mathord{}$}
\def\s#1{LOSSAGE #1}

%%%%%
% temporary definitions RJC 17 Apr '89
...
\def\code{\realbackslash code }%
\def\samp{\realbackslash samp }%
\def\kbd{\realbackslash kbd }%
\def\key{\realbackslash key }%
\def\var{\realbackslash var }%
}

% \indexnofonts no-ops all font-change commands.
% Also make various special chars output as themselves.
...
\count10=\lastpenalty %
% Alas, setting \escapechar here fails to affect the delayed write.
...
% Temporary aliases for old input.
\let\dblarrow=\result
\let\longdblarrow=\expandsto
...
\expandafter\gdef\csname X#1\endcsname {$\langle$un\-def\-in\-ed$\rangle$}%
