/* mailcheck.c The thing that checks the mail files... */
     ...
     char *file;
{
  ...
  if (!mailfiles)
    mailfiles = 
      (FILEINFO **)xmalloc ((mailfiles_count = 1) * sizeof (FILEINFO *));
  else
    mailfiles = 
      (FILEINFO **)xrealloc (mailfiles,
     ...
     char *file;
{
  ...
  if (!disallow_filename_globbing && *file == '~')
