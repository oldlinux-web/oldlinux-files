...
;; makeinfo-recenter-makeinfo-buffer  to redisplay *Makeinfo* buffer
    ...
    (save-excursion
      (save-restriction
        ...
        (let ((search-end (save-excursion (forward-line 100) (point))))
          ...
          ;; Run the `makeinfo-compile' command in the *Makeinfo* buffer
          (save-excursion
            (makeinfo-compile
...
(defun makeinfo-compile (command error-message &optional name-of-mode)
  ...
  (setq makeinfo-compilation-process
	(start-process "makeinfo" "*Makeinfo*"         ; was *compilation*
		       shell-file-name
		       "-c" (concat "exec " command)))
  (with-output-to-temp-buffer "*Makeinfo*"             ; was *compilation*
    (princ "cd ")
  ...
  (let ((regexp compilation-error-regexp))
    (save-excursion
      (set-buffer "*Makeinfo*")                        ; was *compilation*
  ...
  (let* ((thisdir default-directory)
	...
	(or (eq outwin (selected-window))
      ...
      (fundamental-mode)
      (setq mode-name (or name-of-mode "makeinfo"))    ; was `compilation'
  ...
  (cond ((null (buffer-name (process-buffer proc)))
	...
	((memq (process-status proc) '(signal exit))
	 (let* ((obuf (current-buffer))
	   ...
	   (unwind-protect
	       (progn
		 ;; Write something in *Makeinfo* and hack
		 ;; its mode line,
...
(defun makeinfo-recenter-makeinfo-buffer (linenum)
  "Redisplay *Makeinfo* buffer so that most recent output can be seen.
The last line of the buffer is displayed on
line LINE of the window, or centered if LINE is nil."
  (interactive "P")
  (let ((makeinfo-buffer (get-buffer "*Makeinfo*"))
	(old-buffer (current-buffer)))
    (if (null makeinfo-buffer)
	(message "No *Makeinfo* buffer")
