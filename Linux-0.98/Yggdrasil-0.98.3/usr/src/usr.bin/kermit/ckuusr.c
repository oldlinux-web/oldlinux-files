char *userv = "User Interface 5A(088), 12 Oct 92";
...
#endif /* NOSPL */
#ifdef OS2
    "ex",          XXEXI, CM_INV|CM_ABR, /* In OS/2, let "ex" still be EXIT */
#endif /* OS2 */
...
#endif /* MAC */
#endif /* NOFRILLS */
    "g",           XXGET, CM_INV|CM_ABR, /* invisible synonym for GET */
#ifndef NOSPL
...
doping() {
    ...
    if ((x = cmtxt("IP host name or number", line, &s, xxstring)) < 0)
      return(x);
#ifdef MULTINET
    sprintf(line,"multinet ping %s /num=1",s); /* Construct PING command */
#else
    ...
    x = zshcmd(line);
    concb((char)escape);
    return(success = 1);		/* We don't really know the status */
}
#endif /* TCPSOCKET */
