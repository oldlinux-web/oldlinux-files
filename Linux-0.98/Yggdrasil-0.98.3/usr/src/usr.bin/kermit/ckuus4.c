...
extern int
  ...
  zincnt, fdispla, tlevel, xitsta, spmax, insilence;
...
extern char *dialnum, *dialdir, *dialcmd;
	...
	if (binary) {			/* If binary file, */
		...
		if (duplex) {		/* Yes, for half duplex */
			...
			if (x > -1)	/* If no timeout */
			  conoc((char)x); /* display the character, */
...
#else
#ifdef DEC_TCPIP
    printf(" DEC TCP/IP Services for OpenVMS(tm)");
#else
    printf(" None\n");
#endif /* DEC_TCPIP */
#endif /* WINTCP */
...
doshodial() {
    ...
    printf(" Dial dial-string: ");
    s = getdcs(mdmtyp);			/* Ask dial module for it */
    if (s == NULL || !(*s)) {		/* Empty? */
	printf("(none)\n");
    } else {				/* Not empty. */
	while (c = *s++)		     /* Can contain controls */
