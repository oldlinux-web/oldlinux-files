...
extern int mdmspd;
extern char *dialini, *dialdir, *dialcmd;
extern FILE * dialfd;
...
setdial() {
 ...
 use \"%s\" to stand for phone number,\n\
...
/* Serial tty device, possibly modem, connection... */
	...
	} else if (x >= 1 && x <= 4) {	/* Regular COM port */
	    s = os2devtab[x+3].kwd;	/* Use its real name */
...
#ifdef VMS
	    printf("Sorry, device is in use or otherwise unavailable\n");
#else
