...
extern int osize, maxsize, spktl, rpktl, nfils, stdouf, warn, timef, parity;
...
extern int spackets, rpackets, timeouts, retrans, crunched;
...
*/
		int bctsav;		/* Temporary storage */
		bctsav = bctu;		/* Remember current block check type */
		bctu = 1;		/* Set block check to 1 */
		spack('Y',0,(int)strlen((char *)myinit),(CHAR *)myinit);
		logpkt('#',n,(CHAR *)"<reconstructed>"); /* Log it */
		bctu = bctsav;		/* Restore the block check type */
...
#ifdef UNIX
/*
  So far the final turn argument is only for ckutio.c.  Should be added
