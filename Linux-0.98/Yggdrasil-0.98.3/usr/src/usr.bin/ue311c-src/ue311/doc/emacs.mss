...
@pagefooting()
		...
		@i[(C)Copyright 1988, 1989, 1990, 1991 by Daniel M. Lawrence
...
additions to it over the course of the next six years. Updates and
...
of effort into writing complex macros and finding all the bugs in the
macro language before anyone else does.

	A special thanks to Dana Hoggatt who has provided an almost
daily sounnding board for ideas, algorithyms and code. He is responcible
...
@chapter(Basic Concepts)
	...
	The current version of MicroEMACS is 3.11 (Third major
re-write, eleventh public release), and for the rest of this document, we
...
@flushleft(==== MicroEMACS 3.11 [<12] () == fang.txt == File: fang.txt =========)
...
@tag(macros)
	Procedures, or macros, are programs that are used to customize
...
your macros execute. At the moment, up to 512 user variables may be in
...
and must be the first non-wite space placed on a line. Directives
...
@begin(verbatim)
@center(@b[Default Key Bindings for MicroEmacs 3.11])
...
@b(n) lines.  Negative arguments invoke @i(previous-page).
...
next-word@\A negative argument invokes @i(next-word).
...
@b(n) lines.  Negative arguments invoke @i(next-page).
...
scroll-next-down@\Behavior is same as with @i(next-page).

scroll-next-up@\Behavior is same as with @i(previous-page).
...
shrink-window@\A negative argument invokes @i(shrink-window).  An argument
of 0 causes no action.
...
only at this time. Mouse support is provided for the VSII
...
160 columns. 
	...
	The maximum terminal size is 256 columns and 72 row. If you
...
WRONG BUFFER, and if you write out an obsolete buffer, you will loose
earlier edits!
