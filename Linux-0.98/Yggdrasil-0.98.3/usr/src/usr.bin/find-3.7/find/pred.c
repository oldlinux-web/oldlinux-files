...
#define LINK_BUF stat_buf->st_size
	...
	case 'P':		/* pathname with ARGV element stripped */
	  cp = curdepth ? pathname + path_length + 1 : "";
	  fprintf (fp, segment->text, cp);
	  break;
     ...
     struct predicate *pred_ptr;
{
  ...
  if (fstype == NULL || stat_buf->st_dev != current_dev)
    {
      if (fstype)
	free (fstype);
      fstype = filesystem_type (pathname, stat_buf);
      current_dev = stat_buf->st_dev;
    }
  if (strcmp (fstype, pred_ptr->args.str) == 0)
    return (true);
  return (false);
}
