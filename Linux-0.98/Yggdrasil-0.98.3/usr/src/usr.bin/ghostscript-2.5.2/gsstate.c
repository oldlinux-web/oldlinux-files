...
void
gs_state_set_client(gs_state *pgs, char/*void*/ *pdata, const gs_state_client_procs *pprocs)
...
{	gs_state *pnew = alloc_struct(pgs, gs_state, "gs_gsave");
	...
	*pnew = *pgs;
	...
	pgs->saved = pnew;
	pgs->level++;
	if_debug2('g', "[g]save -> 0x%lx, level = %d\n",
...
{	gs_state *saved = pgs->saved;
	char/*void*/ *pdata = pgs->client_data;
	if_debug2('g', "[g]restore 0x%lx, level was %d\n",
		  (ulong)saved, pgs->level);
	if ( !saved ) return gs_gsave(pgs);	/* shouldn't happen */
	pgs->client_data = 0;		/* don't free it */
	free_gstate_contents(pgs);
	*pgs = *saved;
	if ( pgs->client_data != 0 )
	{	if ( pdata != 0 )
			(*pgs->client_procs.copy)(pdata, pgs->client_data);
...
{	gs_state temp;
	...
	p2->saved = p1->saved;
	p1->saved = temp.saved;
	/* Restore the client data pointers. */
	p2->client_data = p1->client_data;
	p1->client_data = temp.client_data;
}
