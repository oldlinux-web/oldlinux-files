...
#define sreturn(code) { s_end_inline(s, sptr, endptr); return(code); }
	...
	case '<':
		if ( scan_enable_level2 )
		   {	c = sgetc_inline(s, sptr, endptr);
			if ( char_is_data(c) ) sputback_inline(s, sptr, endptr);
			switch ( c )
			   {
			case '<':
		   ...
		   {	ref *ref0 = (ref *)(proc_da.base + pstack);
			...
			if ( array_packing.value.index )
			   {	retcode = make_packed_array(ref0, size, myref,
							    "scanner(packed)");
				if ( retcode < 0 ) sreturn(retcode);
				r_set_attrs(myref, a_executable);
			   }
			else
			  {	aref = alloc_refs(size, "scanner(proc)");
				if ( aref == 0 ) sreturn(e_VMerror);
