...
{	return (ulong)dev->height * (gdev_mem_raster(dev) + sizeof(byte *));
}
...
{	byte *scan_line;
	uint raster = mdev->raster = gdev_mem_raster((gx_device_memory *)dev);
	byte **pptr;
	byte **pend;
	if ( mdev->memory_procs.alloc != 0 )
	...
	scan_line = mdev->base;
	pptr = (byte **)(scan_line + (uint)dev->height * raster);
...
gdev_mem_ensure_byte_order(gx_device_memory *dev)
{
	...
	memswab2(dev->base, dev->base, dev->raster * dev->height);
	dev->bytes_le = 0;
#endif
}
...
{	uint bytes_per_line = gx_device_raster(dev, pad_to_word);
	...
	if ( bytes_per_line == mdev->raster )
	   {	if ( swap && pad_to_word >= 0 )
			memswab2(src, dest, bytes_per_line * count);
		else
		...
		for ( c = count; c-- != 0; )
		   {	if ( swap )
			   {	/* We have to take extra care if */
				...
				if ( bytes_per_line & 1 )
				   {	memswab2(src, dest, bytes_per_line - 1);
					dest[bytes_per_line - 1] =
					  src[bytes_per_line];
				   }
				else
					memswab2(src, dest, bytes_per_line);
			   }
			else
