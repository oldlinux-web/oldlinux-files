...
private int show_ctm_setup(P1(gs_show_enum *));
...
gs_widthshow_n_init(register gs_show_enum *penum,
  gs_state *pgs, floatp cx, floatp cy, char chr, const char *str, uint size)
{	int code;
	setup_show_n();
	code = show_setup(penum, pgs, str);
	setup_width();
...
gs_widthshow_init(gs_show_enum *penum,
  gs_state *pgs, floatp cx, floatp cy, char chr, const char *str)
...
/* For kshow, the CTM may have changed, so we have to reestablish */
/* the cached values in the enumerator. */
private int
continue_kshow(register gs_show_enum *penum)
{	int code = show_ctm_setup(penum);
	if ( code < 0 ) return code;
	return show_proceed(penum);
}
