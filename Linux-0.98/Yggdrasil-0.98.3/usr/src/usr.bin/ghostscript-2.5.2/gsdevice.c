...
int
gs_copydevice(gx_device **pnew_dev, const gx_device *dev, proc_alloc_t palloc)
...
#undef shift_mask
	...
	new_dev = (gx_device_memory *)(*mp->alloc)(1, old_dev->params_size, "gs_makeimagedevice(device)");
	if ( new_dev == 0 ) return_error(gs_error_VMerror);
	*new_dev = *old_dev;
	...
	dev_palette = (byte *)(*mp->alloc)(pcount, 1, "gs_makeimagedevice(palette)");
...
{	int was_open = dev->is_open;
	int code = (*dev->procs->put_props)(dev, plist, count);
