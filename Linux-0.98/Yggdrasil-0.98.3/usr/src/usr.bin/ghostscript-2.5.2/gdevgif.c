...
private gx_device_procs gifmono_procs =
  prn_procs(gif_open, gdev_prn_output_page, gdev_prn_close);
...
private gx_device_procs gif8_procs =
  prn_color_procs(gif_open, gdev_prn_output_page, gdev_prn_close,
...
{	int code = gdev_prn_open(pdev);
	...
	puts("The Graphics Interchange Format(c) is");
	puts("the Copyright Property of CompuServe Incorporated.");
	puts("GIF(sm) is a Service Mark property of CompuServe Incorporated.");
...
{	int raster = gdev_prn_raster(pdev);
	...
	int gif_width = raster * (8 / depth);	/* decoders want the width */
						/* on a byte boundary */
