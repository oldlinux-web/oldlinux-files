...
/* src and dest must be 16-bit aligned; count must be even. */
#if !USE_ASM
#undef memswab2				/* see memory_.h */
void
memswab2(const byte *src, byte *dest, int count)
{	register const uint *sptr = (const uint *)src;
	...
	for ( x = count >> 3; --x >= 0; )
	   {	w = sptr[0];
		...
		sptr += 4, dptr += 4;
#else
		dptr[0] = ((w << 8) & 0xff00ff00) + ((w >> 8) & 0x00ff00ff);
		w = sptr[1];
...
/* src and dest must be 32-bit aligned; count must be 0 mod 4. */
/* This routine may be supplanted by assembly code. */
#if !USE_ASM
#undef memswab4				/* see memory_.h */
void
memswab4(const byte *src, byte *dest, int count)
{	register const ulong *sptr = (const ulong *)src;
	register ulong *dptr = (ulong *)dest;
	register int x;
	register uint w0, w1;
	for ( x = count >> 3; --x >= 0; )
	   {	w0 = sptr[0], w1 = sptr[1];
	...
	if ( count & 4 )
	{	w0 = ((const ushort *)sptr)[0];
		w1 = ((const ushort *)sptr)[1];
		((ushort *)dptr)[0] = (w1 >> 8) + (w1 << 8);
		((ushort *)dptr)[1] = (w0 >> 8) + (w0 << 8);
	}
}
#endif					/* !USE_ASM */
