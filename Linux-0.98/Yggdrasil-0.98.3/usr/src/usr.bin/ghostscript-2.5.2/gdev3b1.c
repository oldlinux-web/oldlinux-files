...
gx_device_att3b1 gs_att3b1_device = {
    ...
    0, 0, 0, 0,			/* screen size width and height, page */
#ifdef ATT3B1_PERF
...
att3b1_open(gx_device *dev)
{
	...
	lprintf1("att3b1_open: window WIOCGETD ioctl failed [%d]\n", errno);
	att3b1_close(dev);
	return_error(gs_error_ioerror);
    }
    ...
    att3b1dev->screen_width = (uw.uw_width < att3b1dev->width) ? uw.uw_width
				: att3b1dev->width;
    att3b1dev->screen_height = (uw.uw_height < att3b1dev->width) ? uw.uw_height
				: att3b1dev->height;
    att3b1dev->page = 1;
                      ...
                      gx_color_index colour)
{
    ...
    p = (ushort *)&att3b1dev->screen[(ushort)y*att3b1dev->line_size] + x/16;
    o = x % 16;
    b = 16 - o;
    wl = (w < b) ? w : b;
    maskl = masks[wl] << o;
		 ...
		 gx_color_index colour0, gx_color_index colour1)
{
    ...
    ushort *dst_p, *dst_q;
    ulong bits, mask, *p;
    int src_o, src_b, dst_o, dst_b, op;
    int w1, w2;
     ...
     */
    src_p = ((const ushort *)data) + data_x/16;
    src_o = data_x % 16;
    src_b = 16 - src_o;

    dst_p = (ushort *)&att3b1dev->screen[(ushort)y*att3b1dev->line_size] + x/16;
    dst_o = x % 16;
    dst_b = 16 - dst_o;
...
att3b1_output_page(gx_device *dev, int num_copies, int flush)
{
    ...
    ur.ur_dstx = 0;
    ur.ur_dsty = 0;
    ur.ur_width = att3b1dev->screen_width;
    ur.ur_height = att3b1dev->screen_height;
    ur.ur_srcop = SRCSRC;
    ur.ur_dstop = DSTSRC;
    ur.ur_pattern = 0;
    ...
    ur.ur_srcbase = (ushort *)att3b1dev->screen;
	...
	ut.ut_num = WTXTSLK1;
	sprintf(ut.ut_text,
	    "%s %d, top right (%d,%d), size (%d,%d), press <space> when done.",
	...
	switch(ch) {
	case 'h':
	    xorigin -= att3b1dev->x_pixels_per_inch/4;
	    break;

	case 'k':
	    yorigin -= att3b1dev->y_pixels_per_inch/2;
	    break;
	...
	case 'L':
	    xorigin = att3b1dev->width - att3b1dev->screen_width;
	    break;

	case 'J':
	    yorigin = att3b1dev->height - att3b1dev->screen_height;
	    break;
	...
	if (xorigin >= att3b1dev->width - att3b1dev->screen_width)
	    xorigin = att3b1dev->width - att3b1dev->screen_width;
	if (xorigin < 0)
	    xorigin = 0;
	if (yorigin >= att3b1dev->height - att3b1dev->screen_height)
	    yorigin = att3b1dev->height - att3b1dev->screen_height;
	if (yorigin < 0)
	    yorigin = 0;
    }
