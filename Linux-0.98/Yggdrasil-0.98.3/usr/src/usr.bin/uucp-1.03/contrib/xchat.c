...
char *argv[];
{
  ...
  while(sigs->signal)
    {
      sigs->o_catcher = (sigfunc_t)signal(sigs->signal, sigs->n_catcher);
      sigs += 1;
    }
     ...
     struct script *begin;
{
  ...
  for (curscr = curstate->next; /* point to 1st scr after label */
	...
	case SC_DBOF:
	  ...
	  if ((dbfd = creat (curscr->strprm, 0600)) <= 0)
	    {
	      logit("Failed to create debug log %s", curscr->strprm);
	      perror("");
	      return FAIL;
	    }
  ...
  while (1) 
    {
      chr = xgetc(1);		/* returns upon char input or tmo */
...
/* 
 * usalarm - catches timer alarms
 */
static int usalarm (isig)
int isig;
{
  /* 
   * This is an unsafe call, but what the hell.
   */
  if (!fSalarm_ok)
    logit("Unexpected SIGALRM", "");

  (void) signal (SIGALRM, usalarm);
...
long tmo;
{
  ...
  stimer.it_value.tv_sec = tmo;
  stimer.it_value.tv_usec = 0L;
  imask = sigblock (1 << (SIGALRM - 1));
  fSalarm_ok = TRUE;
...
long msec;
{
  ...
  if(msec == 0)			/* Skip all of this is delay = 0 */
    return;

  stimer.it_interval.tv_sec = 0;
  stimer.it_interval.tv_usec = 0;
  stimer.it_value.tv_sec = msec / 1000L;
  stimer.it_value.tv_usec = (msec % 1000L) * 1000L;
  imask = sigblock (1L << (SIGALRM - 1));
  fSalarm_ok = TRUE;
  if (setitimer (ITIMER_REAL, &stimer, (struct itimerval *) NULL) == 0)
/****    sigpause (imask); ****/
    pause();
...
#if ! HAVE_SETITIMER && ! HAVE_NAPMS && ! HAVE_NAP && ! HAVE_USLEEP
  if(msec == 0)			/* Skip all of this is delay = 0 */
    return;
  sleep (1);
#endif /* ! HAVE_SETITIMER && ! HAVE_NAPMS && ! HAVE_NAP && ! HAVE_USLEEP */
}
