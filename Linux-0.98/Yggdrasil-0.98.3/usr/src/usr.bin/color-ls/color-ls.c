From: dennisf@miles.miles.com (Dennis Flaherty)

	Greetings!

	I made some additional changes to ls.c, after applying
the color-ls patch.  First, I changed the color scheme (yeah,
I know, it's just personal preference, but I like my personal
preferences!).  Second, I made ls look at the basename of argv[0]
to see it's being run as dir or vdir, and edited the Makefile so
you just have three links to an inode instead of three executables
(similar to what was done to mtools).
	Third, and importantly, I changed the declaration of
print_name_with_quoting() and placed the print_color_indicator()
function INSIDE of it.  This makes it possible for colored names
in all cases in which print_name_with_quoting() is called.  It's
most notable if you invoke the long format listing on a directory
...

/* If argv[0] is "dir", the multi-column format is the default regardless
   of the type of output device.

   If argv[0] is "vdir", the long format is the default regardless of the
   type of output device.

   Otherwise, the output format depends on whether the output
   device is a terminal.  This is for the `ls' program. */
     ...
     struct file *f;
{
  ...
  print_name_with_quoting (f->name, f->stat.st_mode);
  ...
  if (f->filetype == symbolic_link)
	...
	{
	  fputs (" -> ", stdout);
	  print_name_with_quoting (f->linkname, f->linkmode);
...

void
print_name_with_quoting (p, mode)
     register char *p;
     unsigned int mode;
{
  ...
  if (print_with_color)
    color_off = print_color_indicator (mode);
