/*  @(#) pl-os.c 1.5.0 (UvA SWI) Jul 30, 1990
...
{ struct termio tio;
  ...
  { case TTY_SAVE:
	...
	tio.c_cc[VEOL2]	= 27;	/* ESC: complete */
	break;
    case TTY_RETYPE:
	tio.c_lflag &= ~ECHO;
	break;
...
#else O_LINE_EDIT
    case TTY_EXTEND_ATOMS:
	chrs.t_brkc = 27;	/* ESC, EOF already on 04 */
	chars_set = TRUE;
	break;
    case TTY_RETYPE:
	flgs.sg_flags &= ~ECHO;
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    Read a character from the terminal.  Note that  SunView   dispatches
    events  when reading from file descriptors.  This is the entry point
    to keep O_PCE happy.
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
{ long waiting;
  ...
  if ( ioctl(0, FIONREAD, &waiting ) != 0)
    fail;

  return waiting > 0;
}
#else O_TERMIOS
#define waiting() (0)		/* not needed on SUNOS_4 */
#endif O_TERMIOS
#endif O_PCE
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    point,  expressing  the  time  to sleep in seconds.  O_PCE complicates
    here as we want to be able to have notifier dispatching and aborts.
- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
#define time_diff(t1, t2) ((t1.tv_sec - t2.tv_sec) * 1000000 +\
...
{ struct timeval timeout;
  ...
  end.tv_sec  = start.tv_sec + (int) time;
  end.tv_usec = start.tv_usec + ((int)(time * 1000000) % 1000000);
  if (end.tv_usec >= 1000000)
  { end.tv_usec -= 1000000;
    end.tv_sec++;
  }

  while(time_diff(end, start) > 250000)	/* wait in 1/4 second intervals */
  { timeout.tv_sec = 0;
...
void
Sleep(time)			/* system has select(), no O_PCE */
real time;
{ struct timeval timeout;
