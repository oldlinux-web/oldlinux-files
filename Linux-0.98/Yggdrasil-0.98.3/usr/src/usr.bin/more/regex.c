...
static char *allocap = (char *)0;
static int allocasize = 0;
     ...
     int mstop;
{
 ...
 /* Failure point stack.  Each place that can handle a failure further down the line
  ...
  if (!allocap) {
    allocap =  (char *)malloc (2 * NFAILURES * sizeof (char *));
    allocasize = 2 * NFAILURES * sizeof (char *);
  } 
  stackb = (char **) allocap;
  stackp = stackb;
  stacke = (char **) (allocap + allocasize);
 ...
 while (d == dend)						    \
  ...
  while (1)
	...
	case on_failure_jump:
	  if (stackp == stacke)
	    {
	      allocap = (char *) realloc (allocap, 2 * allocasize);
	      allocasize *= 2;
	      stackp += ((char **) allocap) - stackb;
	      stackb = (char **) allocap;
	      stacke = (char **) (allocap + allocasize);
	    }
	  mcnt = *p++ & 0377;
