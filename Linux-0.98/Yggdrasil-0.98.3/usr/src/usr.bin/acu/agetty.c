...
char *progname;
void lock_port( char *);
#define ZLOG "/dev/tty1"
#ifdef DEBUGGING
#define debug(s) fopen( ZLOG, "w" ); fprintf(dbf,s); fflush(dbf); fclose( dbf)
#define debug2(s, r) fopen( ZLOG, "w" ); fprintf(dbf,(s),(r)); \
                     fflush(dbf);fclose(dbf)
FILE *dbf;
#else
...
debug( "YOW\n");
...
#ifdef linux
	{
		...
		iv = getpid();
		iv = ioctl(0, TIOCSPGRP, &iv);
		debug2( "IOCTL returns %d\n\r", iv );
	}
#endif
...
char buffer[128];
	...
	strcpy( buffer, "/usr/spool/uucp/LCK.." );
	strcat( buffer, cptr    );

/*
	really should check if file DOES EXIST already ...
	but if someone else is using the port we ALWAYS have priority
	... we're getty!
*/
  ...
  (void) chown(buffer, 0, 0);			/* root, sys */
  (void) chmod(buffer, 0666);			/* crw--w--w- */
