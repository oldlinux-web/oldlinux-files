...
#ifndef __STDC__
/* Assume fprintf() was defined in stdio.h */
extern int fprintf();
#endif
...
#ifdef LINUX_V0_12_PAUSE_BUG
	while (seconds_to_wait > 0)
	{
		Debug(DSCH, ("[%d] sleeping for %d seconds\n",
			getpid(), seconds_to_wait))
		/* The sleep may end; due to some strange, caught */
		/* signal.  In that event; sleep returns the amount */
		/* of time remaining to be slept. */
		seconds_to_wait =  sleep((unsigned int) seconds_to_wait);
		Debug(DSCH, ("\n[%d] done sleeping, %d seconds remaining\n", 
                      getpid(), seconds_to_wait));
	}
#else
