...
#if defined(ATT)
     /* Be sure to avoid the SIGCHLD here; so as not to wake up the parent */
     /* when the children die; some SYSVs actually do this... */
#ifdef SIGCHLD
	(void) signal(SIGCHLD, SIG_IGN);
#endif
...
#ifdef ATT
		if ((waiter & 0xff00) == 0 && /* Hi order 8 bits are zero */
		    (waiter & 0x80) == 1) /* 7th low-order bit */ 
